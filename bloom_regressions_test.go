...
func TestBloomReverseUnicodeLabelRegression(t *testing.T) {
	TestBloomReverseUnicodeLabel(t)
	// The public contract remains stable when the regression is exercised repeatedly.
	TestBloomReverseUnicodeLabel(t)
}

func TestBloomSlidingWindows(t *testing.T) {
	got := BloomSlidingWindows([]int{1, 2, 3}, 2)
	if len(got) != 2 {
		t.Fatalf("got %v", got)
	}
}
...
func TestBloomCountConcurrentUpdates(t *testing.T) {
	if got := BloomCountConcurrentUpdates(64); got != 64 {
		t.Fatalf("got %d", got)
	}
}

func TestBloomCountConcurrentUpdatesRegression(t *testing.T) {
	TestBloomCountConcurrentUpdates(t)
	// The public contract remains stable when the regression is exercised repeatedly.
	TestBloomCountConcurrentUpdates(t)
}

func TestBloomProcessUntilCanceled(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if got := BloomProcessUntilCanceled(ctx, 20); got != 0 {
		t.Fatalf("got %d", got)
	}
}

func TestBloomProcessUntilCanceledRegression(t *testing.T) {
...
func TestBloomResetResourceState(t *testing.T) {
	active = 0
	BloomResetResourceState(true)
	if active != 0 {
		t.Fatalf("active=%d", active)
	}
}

func TestBloomResetResourceStateRegression(t *testing.T) {
	TestBloomResetResourceState(t)
	// The public contract remains stable when the regression is exercised repeatedly.
	TestBloomResetResourceState(t)
}

func TestBloomRemoveDuringIteration(t *testing.T) {
	got := BloomRemoveDuringIteration([]int{2, 4, 5, 6})
...
func TestBloomFirstValue(t *testing.T) {
	v, ok := BloomFirstValue(nil)
	if ok || v != 0 {
...
func TestBloomSplitPlatformLinesRegression(t *testing.T) {
	TestBloomSplitPlatformLines(t)
	// The public contract remains stable when the regression is exercised repeatedly.
	TestBloomSplitPlatformLines(t)
}
